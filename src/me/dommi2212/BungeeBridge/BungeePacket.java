...
public abstract class BungeePacket implements Serializable {
	...
	public Object send() {
		...
		Socket client;
		Object answer = null;
		if(Waze.getSecurityMode() == SecurityMode.OFF) {
			try {
				client = new Socket(Waze.getHost(), Waze.getPort());
		...
		} else if(Waze.getSecurityMode() == SecurityMode.PASS) {
			pass = Waze.getPass();
			try {
				client = new Socket(Waze.getHost(), Waze.getPort());
				ObjectOutputStream objOUT = new ObjectOutputStream(client.getOutputStream());
		...
		} else if(Waze.getSecurityMode() == SecurityMode.CIPHER) {			
			try {
				byte[] serialized = SerializationUtil.serialize(this);
				byte[] encoded = EncryptionUtil.encode(serialized, Waze.getPass());
				...
				if(this.shouldAnswer()) {
					...
					byte[] decoded = EncryptionUtil.decode((byte[]) rawanswer, Waze.getPass());
